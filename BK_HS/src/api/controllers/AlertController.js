...
class AlertController {
  ...
  async createRule(req, res) { const result = await this.createAlertRule.execute({ userId: req.user.id, homeId: req.params.homeId, ...req.body }); res.status(201).json({ data: result }); }
  ...
  async updateRule(req, res) { const result = await this.updateAlertRule.execute({ userId: req.user.id, ruleId: req.params.ruleId, ...req.body }); res.json({ data: result }); }
  ...
  async saveThreshold(req, res) { const result = await this.saveThresholdConfiguration.execute({ userId: req.user.id, homeId: req.params.homeId, ...req.body }); res.json({ data: result }); }
