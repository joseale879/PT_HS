...
const GROUPS = new Set(['daily', 'hourly', 'monthly', 'location']);

class GetConsumptionSeries {
  constructor({ consumptionRepository }) {
    this.consumptionRepository = consumptionRepository;
  }

  execute({ userId, homeId, from, to, groupBy = 'daily' }) {
    ...
    if (!this.isDate(from) || !this.isDate(to)) {
      throw this.badRequest('from y to deben tener formato YYYY-MM-DD');
    }
    ...
    const normalizedGroup = String(groupBy || 'daily').trim().toLowerCase();
    if (!GROUPS.has(normalizedGroup)) {
      throw this.badRequest('groupBy debe ser daily, hourly, monthly o location');
    }
    assertMaxPeriodDays(from, to);

    return this.consumptionRepository.getSeries({
      userId,
      homeId,
      from,
      to,
...
module.exports = { GetConsumptionSeries };
