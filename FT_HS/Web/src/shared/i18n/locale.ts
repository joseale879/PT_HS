...
const languageSettings: Record<
  SupportedLanguage,
  { locale: string; defaultCurrency: string; flag: string }
> = {
...
export const languageOptions = (Object.keys(languageSettings) as SupportedLanguage[]).map(
  (code) => ({ code, ...languageSettings[code] })
);
