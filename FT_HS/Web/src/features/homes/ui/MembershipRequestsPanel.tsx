...
export function MembershipRequestsPanel() {
  ...
  useEffect(() => {
    homesApi.list()
      ...
      .catch((error) => toast.error(error instanceof Error ? error.message : 'No se pudieron cargar los hogares'))
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Envía una solicitud para unirte a un hogar o gestiona las solicitudes pendientes de tus hogares.</CardDescription>
      </CardHeader>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
          <div className="flex-1 space-y-2">
            ...
            <Select value={selectedHomeId} onValueChange={setSelectedHomeId}>
              <SelectTrigger id="membership-home"><SelectValue placeholder="Selecciona un hogar" /></SelectTrigger>
              <SelectContent>
                {homes.map((home) => <SelectItem key={home.homeId} value={home.homeId}>{home.name}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <Button type="button" variant="outline" onClick={requestAccess} disabled={busyId !== null}>
            <UserPlus className="mr-2 size-4" /> Solicitar ingreso
          </Button>
        </div>

        {!requests.length ? (
          <Alert><AlertDescription>No hay solicitudes registradas para este hogar.</AlertDescription></Alert>
        ) : requests.map((request) => (
          <div key={request.requestId} className="flex flex-col gap-3 rounded-lg border p-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-sm font-medium">{request.fullName || request.email || `Usuario ${request.userId}`}</p>
              {request.email && <p className="text-xs text-gray-600">{request.email}</p>}
              <p className="text-xs text-gray-500">Solicitada: {formatDate(request.requestedAt)}</p>
            </div>
            <div className="flex items-center gap-2">
              <Badge variant={request.status === 'Pending' ? 'secondary' : 'outline'}>{request.status}</Badge>
              {request.status === 'Pending' && (
                <>
                  <Button type="button" size="sm" onClick={() => answerRequest(request.requestId, 'Approved')} disabled={busyId !== null}>
                    <Check className="mr-1 size-4" /> Aprobar
                  </Button>
                  <Button type="button" size="sm" variant="outline" onClick={() => answerRequest(request.requestId, 'Rejected')} disabled={busyId !== null}>
