...
export function LoginScreen({
  ...
  const [resetContextStatus, setResetContextStatus] = useState<
  ...
  useEffect(() => {
    if (!isResetFromEmail) {
      setResetContextStatus('idle');
      return;
    }

    let cancelled = false;
    setResetContextStatus('loading');
    onPasswordResetContext(resetToken)
      .then((response) => {
        if (cancelled) return;
        setResetEmail(response.data.email);
        setResetContextStatus('valid');
      })
      .catch(() => {
        if (cancelled) return;
        setResetContextStatus('invalid');
      });

    return () => {
      cancelled = true;
    };
  }, [isResetFromEmail, onPasswordResetContext, resetToken]);
                    ...
                    <DialogContent>
                      ...
                      <div className="space-y-4 py-4">
                        {isResetFromEmail ? (
                          <>
                            <div className="space-y-2">
                              <Label htmlFor="resetEmailVerified">{t('auth.email')}</Label>
                              <Input
                                id="resetEmailVerified"
                                type="email"
                                value={
                                  resetContextStatus === 'loading'
                                    ? 'Validando enlace...'
                                    : resetEmail
                                }
                                readOnly
                                disabled={resetContextStatus !== 'valid'}
                              />
                            </div>
                            {resetContextStatus === 'invalid' && (
                      ...
                      <DialogFooter>
                        ...
                        {isResetFromEmail ? (
                          <Button
                            ...
                            disabled={isResettingPassword || resetContextStatus !== 'valid'}
                          >
