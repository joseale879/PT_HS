-- Restore the normalized registration function that existed before this changeset.

DROP FUNCTION IF EXISTS device.fn_register_device(VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR, DECIMAL);
DROP FUNCTION IF EXISTS user_account.fn_assign_default_role(UUID);
