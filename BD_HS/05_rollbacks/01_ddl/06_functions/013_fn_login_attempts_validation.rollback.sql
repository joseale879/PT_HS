...
BEGIN
    ...
    IF v_blocked_until IS NOT NULL AND v_blocked_until > now() THEN
        ...
        RETURN;
    END IF;
    v_failed_attempts := CASE WHEN v_blocked_until IS NULL THEN COALESCE(v_failed_attempts, 0) + 1 ELSE 1 END;
    v_blocked_until := CASE WHEN v_failed_attempts >= v_lockout_attempts THEN now() + make_interval(mins => v_lockout_minutes) ELSE NULL END;
    INSERT INTO user_account.login_attempt (user_account_id, failed_attempts, blocked_until, last_attempt_at, source_ip)
    VALUES (p_user_account_id, v_failed_attempts, v_blocked_until, now(), p_source_ip)
