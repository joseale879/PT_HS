...
BEGIN
    ...
    v_month_start := date_trunc('month', NEW.recorded_at);
    v_month_end := v_month_start + INTERVAL '1 month';
    v_is_vacation := home.fn_is_vacation_active(
        NEW.home_id,
           ...
           AND NOT EXISTS (
               SELECT 1
                ...
                WHERE ae.rule_id = v_monthly_rule_id
                  AND ae.home_id = NEW.home_id
                  AND ae.status = 'Pending'
                  AND ae.generated_at >= v_month_start
                  AND ae.generated_at < v_month_end
           ) THEN
        ...
        IF v_leak_rule_id IS NOT NULL
           ...
           AND alert_rate.fn_is_leak_detected(NEW.device_id, NEW.recorded_at, 30) THEN
            INSERT INTO alert_rate.alert_event (
                ...
                'Posible fuga detectada.', NEW.consumption_m3, 'Pending'
            );
        END IF;
    END IF;
