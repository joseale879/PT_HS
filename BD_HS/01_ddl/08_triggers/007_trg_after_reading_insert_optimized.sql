...
BEGIN
    ...
    IF NOT v_is_vacation THEN
        ...
        SELECT rule_id INTO v_daily_rule_id
        FROM alert_rate.alert_rule
        WHERE home_id = NEW.home_id AND alert_type = 'daily_limit' AND active = TRUE
        LIMIT 1;
        ...
        IF v_daily_limit IS NOT NULL AND v_daily_rule_id IS NOT NULL
           AND NOT EXISTS (
               ...
               WHERE ae.rule_id = v_daily_rule_id
                 ...
                 AND ae.generated_at >= v_day_start
                 AND ae.generated_at < v_day_end
           ) THEN
            v_daily_consumption := consumption.fn_get_daily_consumption(
                NEW.home_id,
                (NEW.recorded_at AT TIME ZONE current_setting('TimeZone'))::date
            ) / 1000.0;

            IF v_daily_consumption > v_daily_limit THEN
                INSERT INTO alert_rate.alert_event (
                    event_id, rule_id, home_id, message, detected_value, status
                ) VALUES (
                    gen_random_uuid(), v_daily_rule_id, NEW.home_id,
                    'Tope diario de consumo superado.',
                    v_daily_consumption, 'Pending'
                );
            END IF;
        END IF;

        SELECT rule_id INTO v_monthly_rule_id
        FROM alert_rate.alert_rule
        WHERE home_id = NEW.home_id AND alert_type = 'monthly_limit' AND active = TRUE
        LIMIT 1;
           ...
           AND NOT EXISTS (
            ...
            IF v_monthly_consumption > v_monthly_limit THEN
                INSERT INTO alert_rate.alert_event (
                    ...
                    gen_random_uuid(), v_monthly_rule_id, NEW.home_id,
                    'Tope mensual de consumo superado.',
                    v_monthly_consumption, 'Pending'
                );
            END IF;
        END IF;

        SELECT rule_id INTO v_leak_rule_id
        FROM alert_rate.alert_rule
        WHERE home_id = NEW.home_id AND alert_type = 'leak_detected' AND active = TRUE
        LIMIT 1;
        ...
        IF v_leak_rule_id IS NOT NULL
           ...
           AND alert_rate.fn_is_leak_detected(NEW.device_id, NEW.recorded_at, 30) THEN
            INSERT INTO alert_rate.alert_event (
                ...
                gen_random_uuid(), v_leak_rule_id, NEW.home_id,
                'Posible fuga detectada.',
                NEW.consumption_m3, 'Pending'
            );
        END IF;
    END IF;
